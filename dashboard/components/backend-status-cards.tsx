...
function statusFromCount(n: number): BackendStatus {
  // Hackathon-simple heuristic:
  // - if this backend got traffic, it's healthy
  // - if it got none, it's degraded (or down if you want)
...
export function BackendStatusCards() {
  ...
  const counts = data?.per_backend ?? { A: 0, B: 0, C: 0 };
  ...
  const backends: Backend[] = [
    {
      ...
      key: "A",
      url: "A (Round-Robin)",
      status: statusFromCount(counts.A),
      responseTime: 45, // placeholder until real forward + measure
      activeConnections: counts.A,
      loadWeight: 33, // placeholder
    ...
    },
    {
      ...
      key: "B",
      url: "B (Round-Robin)",
      status: statusFromCount(counts.B),
      responseTime: 52, // placeholder
      activeConnections: counts.B,
      loadWeight: 33, // placeholder
      lastHealthCheck: new Date(),
    },
    {
      ...
      key: "C",
      url: "C (Round-Robin)",
      status: statusFromCount(counts.C),
      responseTime: 49, // placeholder
      activeConnections: counts.C,
      loadWeight: 34, // placeholder
      lastHealthCheck: new Date(),
    },
  ];
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge variant="outline" className={statusStyles.badgeClass}>
                    <span
                      className={`mr-1.5 h-2 w-2 rounded-full ${statusStyles.dotClass}`}
                    />
                    {statusStyles.label}
                  </Badge>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium">
                        {formatRelativeTime(backend.lastHealthCheck)}
                      </p>
                    </div>
                  </div>
                </div>
